...
def generate_from_sample(sample: dict) -> dict:
    ...
    """
    faker = Faker()
    return _regenerate(sample, field_name="", faker=faker)


def _regenerate(value: object, field_name: str, faker: Faker) -> object:
    """Recursively regenerate a value based on its type and field name."""
    if isinstance(value, dict):
        return {k: _regenerate(v, field_name=k, faker=faker) for k, v in value.items()}

    if isinstance(value, list):
        if not value:
            return []
        return [_regenerate(item, field_name=field_name, faker=faker) for item in value]
